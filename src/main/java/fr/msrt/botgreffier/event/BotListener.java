    ...
    private void onMessage(GuildMessageReceivedEvent event) {
        ...
        try {
            ...
            if (event.getGuild().getSelfMember().hasPermission(Permission.MESSAGE_WRITE)) {
                ...
                /* if (msg.equalsIgnoreCase("=greffier")) {
                    objChannel.sendMessage("Oui c'est moi <:ouicestmoi:503244521295839244>").complete();
                    objChannel.sendTyping().complete();
                    delay(500);
                    objChannel.sendMessage("Je suis " + event.getJDA().getSelfUser().getAsMention() + ", un bot Discord.").complete();
                    objChannel.sendTyping().complete();
                    delay(500);
                    objChannel.sendMessage("Incroyable n'est-ce pas ?").complete();
                    sysoutCmd(msg);
                    return;
                } */

                if (msg.equalsIgnoreCase("=bonjour") || msg.equalsIgnoreCase("=bjr")) {
                    objChannel.sendMessage("Bonjour " + objUser.getAsMention() + " !").queue();
                    sysoutCmd(msg);
                    return;
                }

                if (msg.equalsIgnoreCase("=salut") || msg.equalsIgnoreCase("=slt")) {
                    objChannel.sendMessage("Salut " + objUser.getAsMention() + " !").queue();
                    sysoutCmd(msg);
                    return;
                }

                if (msg.equalsIgnoreCase("=help") || msg.equalsIgnoreCase("=aide")) {
                    objChannel.sendMessage("**Liste des commandes :** https://msrt.ml/botgreffier/commandes").queue();
                    sysoutCmd(msg);
                    return;
                }

                if (msg.equalsIgnoreCase("=LS2k17") || msg.equalsIgnoreCase("=lien")) {
                    objChannel.sendMessage("<:chat:447506024388231197> https://ls2k17.wordpress.com").queue();
                    sysoutCmd(msg);
                    return;
                }

                if (msg.equalsIgnoreCase("=télécharger") || msg.equalsIgnoreCase("=telecharger")) {
                    objChannel.sendMessage("<:chat:447506024388231197> https://ls2k17.wordpress.com/telecharger").queue();
                    sysoutCmd(msg);
                }

                if (msg.equalsIgnoreCase("=chapon") || msg.equalsIgnoreCase("=chapon.ga")) {
                    objChannel.sendMessage("<:chapon:452484558055538708> https://chapon.ga").queue();
                    sysoutCmd(msg);
                }

                if (msg.equalsIgnoreCase("=snapchat") || msg.equalsIgnoreCase("=snap") || msg.equalsIgnoreCase("=lens")) {
                    objChannel.sendMessage("http://bit.ly/SnApcode").queue();
                    objChannel.sendMessage("**Ou cliquez sur ce lien depuis un téléphone :** https://www.snapchat.com/unlock/?type=SNAPCODE&uuid=b8cf7b00dbd04874ba0a5657b73038ae&metadata=1").queue();
                    sysoutCmd(msg);
                    return;
                }

                if (msg.equalsIgnoreCase("=test")) {
                    objChannel.sendMessage("Ça fonctionne :ok_hand:").queue();
                    sysoutCmd(msg);
                    return;
                }

                if (msg.equalsIgnoreCase("=bot")) {
                    objChannel.sendMessage("<:bot:449319834078150676>").queue();
                    sysoutCmd(msg);
                    return;
                }

                if (msg.equalsIgnoreCase("=ver") || msg.equalsIgnoreCase("=version")) {
                    objChannel.sendMessage("Version du bot : " + Info.getVer()).queue();
                    sysoutCmd(msg);
                    return;
                }

                if (msg.equalsIgnoreCase("=maj") || msg.equalsIgnoreCase("=màj") ||  msg.equalsIgnoreCase("=miseajour") || msg.equalsIgnoreCase("=miseàjour")) {
                    objChannel.sendMessage("Dernière mise à jour : " + Info.getMaj()).queue();
                    sysoutCmd(msg);
                    return;
                }

                if (msg.equalsIgnoreCase("=instance") || msg.equalsIgnoreCase("=nom")) {
                    objChannel.sendMessage("Nom de l'instance : " + Info.getInstanceName()).queue();
                    sysoutCmd(msg);
                    return;
                }

                if ((msg.equalsIgnoreCase("=vaderetro")) || (msg.equalsIgnoreCase("=vaderetrosatanas")) || (msg.equalsIgnoreCase("=satanas"))) {
                    objChannel.sendMessage("https://chapon.ga/i/vaderetro.jpg").queue();
                    sysoutCmd(msg);
                    return;
                }

                if ((msg.equalsIgnoreCase("=oeuf")) || (msg.equalsIgnoreCase("=œuf"))) {
                    objChannel.sendMessage(":egg:").queue();
                    sysoutCmd(msg);
                    return;
                }
